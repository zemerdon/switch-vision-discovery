...
RESOLVER="$RUNTIME/physical_contract.py"
...
run_case() {
  ...
  if ! SWITCH_VISION_RUNTIME_DIR="$RUNTIME" \
       ...
       sh "$PREPARE" "$walk" "$normalized" "$capabilities" "$contract" > "$case_dir/prepare.out" 2> "$case_dir/prepare.err"; then
    note_failure "$case_name: physical contract resolver rejected evidence"
    sed -n '1,120p' "$case_dir/prepare.err" >&2 || true
    [ -f "$contract" ] && cat "$contract" >&2 || true
    return 0
  fi

  contract_status=$(jq -r '.status // "missing"' "$contract")
  contract_model=$(jq -r '.device.model // "missing"' "$contract")
  contract_physical=$(jq -r '.observed.physical // 0' "$contract")
  [ "$contract_status" = "resolved" ] || note_failure "$case_name: contract status '$contract_status' != resolved"
  [ "$contract_model" = "$expected_model" ] || note_failure "$case_name: contract model '$contract_model' != '$expected_model'"
  [ "$contract_physical" = "$expected_physical" ] || note_failure "$case_name: contract physical '$contract_physical' != '$expected_physical'"

  printf '%s,192.0.2.10,%s,readonly,,%s\n' "$(basename "$normalized")" "$prefix" "$case_name" > "$targets"
  cat > "$options" <<EOF
{
...
EOF
  ...
  printf '%-12s contract=%-8s model=%-28s physical=%-4s mapped=%-4s yaml_status=%s\n' \
    "$case_name" "$contract_status" "${actual_model:-missing}" "${actual_physical:-missing}" "${actual_mapped:-missing}" "${yaml_status:-0}"
...
# Known-good control: resolver adoption must preserve the working HP 44+4 path.
