...
function syncGeneratedDashboardExportMode(){const custom=generatedDashboardExportMode()==='custom-dashboard';const link=$('downloadGeneratedDashboardYamlButton');if(link)link.href=endpoint(custom?'download/switch-vision-custom-dashboard.yaml':'download/switch-vision-dashboard.yaml');const status=$('generatedCardYamlActionStatus');if(status&&!$('generatedCardYamlPreview')?.classList.contains('hidden'))status.textContent=`Preview format changed to ${custom?'Custom dashboard (Layout Card)':'Standard dashboard'}. Click Preview Dashboard YAML to refresh.`}
