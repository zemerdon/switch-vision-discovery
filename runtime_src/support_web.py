...
_COMPONENT_CATALOG_CACHE: tuple[float, tuple[tuple[str, str, str], ...]] | None = None
_INSTALLER_COMPONENT_CATALOG_URL = "https://raw.githubusercontent.com/zemerdon/switch-vision-installer/main/switch_vision_installer/app/component_manager.py"

def _installer_component_catalog() -> tuple[tuple[str, str, str], ...]:
    global _COMPONENT_CATALOG_CACHE
    now = time.time()
    if _COMPONENT_CATALOG_CACHE and now - _COMPONENT_CATALOG_CACHE[0] < 300:
        return _COMPONENT_CATALOG_CACHE[1]
    request = Request(_INSTALLER_COMPONENT_CATALOG_URL, headers={"User-Agent": "Switch-Vision-Hub"})
    try:
        with urlopen(request, timeout=8.0) as response:
            source = response.read().decode("utf-8")
        block = source.split("COMPONENTS:", 1)[1].split("\n_CACHE:", 1)[0]
        pattern = re.compile(r'ComponentSpec\(\s*"([^"]+)"\s*,\s*"([^"]+)"\s*,\s*"[^"]+"\s*,\s*\(\s*"([A-Za-z0-9_.-]+)"\s*,?\s*\)', re.S)
        rows = tuple(pattern.findall(block))
        if not rows or len({row[0] for row in rows}) != len(rows):
            raise RuntimeError("invalid Installer component catalog")
    except Exception as exc:
        raise RuntimeError("Switch Vision component catalog is unavailable.") from exc
    _COMPONENT_CATALOG_CACHE = (now, rows)
    return rows

def _public_release_for_repo(repository: str) -> dict[str, Any]:
    now = time.time()
    cached = _PUBLIC_RELEASE_CACHE.get(repository)
    if cached and now - cached[0] < 300:
        return dict(cached[1])
    request = Request(
        f"https://api.github.com/repos/zemerdon/{quote(repository, safe='')}/releases/latest",
        headers={"Accept": "application/vnd.github+json", "User-Agent": "Switch-Vision-Hub"},
    )
    try:
        with urlopen(request, timeout=8.0) as response:
            payload = json.loads(response.read().decode("utf-8"))
        ...
        result = {
            "available": bool(version and published_at),
        ...
        result = {
            ...
            "published_at": None,
            "url": None,
            "error": f"{type(exc).__name__}: release metadata unavailable",
        }
    _PUBLIC_RELEASE_CACHE[repository] = (now, dict(result))
    return result

def _switch_vision_component_releases() -> dict[str, Any]:
    components = []
    for key, label, repository in _installer_component_catalog():
        release = _public_release_for_repo(repository)
        components.append({
            "key": key,
            "label": label,
...
<div class="hub-settings-actions"><button id="hubSettingsSave" class="primary" type="button" disabled>Save changes</button><button id="hubSettingsReload" type="button">Reload</button>
      <button id="hubSettingsBack" type="button">Back</button><p id="hubSettingsStatus" class="muted hub-settings-status">Open a section to review settings.</p></div>
</section>
...
class SupportHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        elif path == "/api/component-releases":
            self._json(_switch_vision_component_releases())
