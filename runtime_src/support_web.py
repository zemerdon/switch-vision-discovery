...
def _test_unifi2mqtt_connection(data: Any) -> dict[str, Any]:
    ...
    if transport == "local":
        ...
        verify_ssl = _bool_string(
            data.get("verify_ssl", current.get("local_verify_ssl", "true")),
            "local_verify_ssl",
        ) == "true"
        base = controller_url
...
def _generated_dashboard_export(path: Path, *, cards_only: bool = False) -> dict[str, Any]:
    ...
    Manual exports remove the known third-party vertical-layout view wrapper so a
    newly pasted Home Assistant dashboard has no Layout Card dependency.
    """
    ...
    for view in views:
        ...
        if view_type == "custom:vertical-layout":
            ...
            return {"valid": False, "error": f"Dashboard export cannot remove unknown custom view dependency: {view_type}"}
        else:
...
/* UI/UX consistency layer: preserve capability while improving hierarchy, feedback, and narrow-view ergonomics. */
:root{--sv-feedback-radius:9px;--sv-touch-target:44px}
.sv-feedback{display:flex;align-items:center;gap:8px;min-height:34px;margin:8px 0;padding:8px 10px;border:1px solid var(--line-soft);border-left-width:4px;border-radius:var(--sv-feedback-radius);background:var(--surface-inset);color:var(--text);line-height:1.3;overflow-wrap:anywhere}.sv-feedback:empty{display:none}.sv-feedback.is-info{border-left-color:var(--accent);background:var(--accent-soft)}.sv-feedback.is-loading{border-left-color:var(--accent);background:var(--accent-soft)}.sv-feedback.is-success{border-left-color:var(--ok);background:var(--ok-soft)}.sv-feedback.is-warning{border-left-color:var(--warn);background:var(--warn-soft)}.sv-feedback.is-error{border-left-color:var(--bad);background:var(--bad-soft)}.sv-feedback.is-loading::before{content:"";width:12px;height:12px;flex:0 0 12px;border:2px solid var(--line);border-top-color:var(--accent);border-radius:50%;animation:sv-feedback-spin .8s linear infinite}@keyframes sv-feedback-spin{to{transform:rotate(360deg)}}
.device-summary-title-row{display:flex;align-items:center;gap:8px;flex-wrap:wrap}.device-summary-title-row>strong{font-size:1rem;color:var(--heading-strong)}.device-health-chip{display:inline-flex;align-items:center;padding:2px 7px;border:1px solid var(--line);border-radius:999px;font-size:var(--sv-font-small);font-weight:750;line-height:1.35}.device-health-chip.is-enabled{border-color:var(--ok);background:var(--ok-soft);color:var(--ok)}.device-health-chip.is-disabled{background:var(--neutral-soft);color:var(--muted)}.device-summary-facts{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:6px;margin-top:8px}.device-summary-fact{min-width:0;padding:6px 8px;border:1px solid var(--line-soft);border-radius:8px;background:var(--chip-bg)}.device-summary-fact-label{display:block;color:var(--muted);font-size:var(--sv-font-small);line-height:1.15}.device-summary-fact-value{display:block;margin-top:2px;color:var(--text);font-weight:650;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.device-details-label{font-size:var(--sv-font-small);font-weight:700}.unified-device-disclosure{width:auto!important;min-width:74px!important;gap:6px!important}.configured-management{overflow-wrap:anywhere}.unified-device-actions{flex-wrap:wrap}.device-card,.diag-tile,.yaml-manager,.hub-settings-section{transition:border-color .15s ease,background-color .15s ease,box-shadow .15s ease}.device-card:focus-within{border-color:var(--accent-strong);box-shadow:0 0 0 2px var(--accent-soft)}
.hub-settings-advanced{padding:0!important;overflow:hidden}.hub-settings-advanced>summary{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:11px 12px;cursor:pointer;list-style:none;color:var(--heading);font-weight:750}.hub-settings-advanced>summary::-webkit-details-marker{display:none}.hub-settings-advanced>summary::marker{content:""}.hub-settings-advanced>summary::after{content:"Advanced  ▸";color:var(--muted);font-size:var(--sv-font-small);font-weight:700;transition:transform .12s ease}.hub-settings-advanced[open]>summary{border-bottom:1px solid var(--line-soft);background:var(--heading-soft)}.hub-settings-advanced[open]>summary::after{content:"Advanced  ▾"}.hub-settings-advanced>:not(summary){margin-left:12px;margin-right:12px}.hub-settings-advanced>:last-child{margin-bottom:12px}.hub-settings-advanced>.muted{margin-top:9px}.hub-settings-advanced>.grid{margin-top:9px}
@media(max-width:900px){body[class*="width-"] main{width:100%!important}.device-summary-facts{grid-template-columns:repeat(2,minmax(0,1fr))}.diag-summary{grid-template-columns:repeat(2,minmax(0,1fr))}}
@media(max-width:760px){body{padding:12px}.card{margin:12px 0;padding:14px}.unified-device-summary,.unified-device-details>summary{display:grid;grid-template-columns:auto minmax(0,1fr);align-items:start;gap:8px 10px}.device-order-controls{grid-column:1;grid-row:1;align-self:start}.unified-device-main{grid-column:2;grid-row:1;min-width:0}.unified-device-actions{grid-column:1/-1;grid-row:2;width:100%;justify-content:flex-start}.unified-device-actions .device-state-toggle{flex:1 1 150px}.unified-device-actions .unified-device-disclosure{flex:0 0 auto}.device-summary-facts{grid-template-columns:repeat(2,minmax(0,1fr))}.hub-settings-tabs{scroll-snap-type:x proximity}.hub-settings-tab{scroll-snap-align:start}.hub-settings-actions{bottom:0}.live-status{grid-template-columns:1fr}.live-status dt{margin-top:5px}.live-status dd{padding-bottom:5px;border-bottom:1px solid var(--line-soft)}.live-status dd:last-child{border-bottom:0}.topbar{align-items:flex-start}.topbar-tools{justify-content:flex-start}}
@media(max-width:520px){body{padding:8px}.card{border-radius:11px;padding:11px}.topbar h1{min-width:0;width:100%}.topbar-tools{width:100%;display:grid;grid-template-columns:1fr auto}.theme-picker{min-width:0}.theme-picker select{min-width:0;width:100%}.actions{display:grid;grid-template-columns:1fr;width:100%}.actions>button,.actions>a.button{width:100%;min-height:var(--sv-touch-target)}button,a.button,select,input[type=text],input[type=file],input[type=number],input[type=password],input[type=url]{min-height:var(--sv-touch-target)}.device-summary-facts,.diag-summary{grid-template-columns:1fr}.unified-device-summary,.unified-device-details>summary{padding:10px}.unified-device-body{padding:10px}.device-order-controls{gap:4px}.device-order-controls button{min-width:var(--sv-touch-target);min-height:var(--sv-touch-target)}.unified-device-actions{display:grid;grid-template-columns:1fr auto;align-items:stretch}.unified-device-actions .device-state-toggle{width:100%}.unified-device-disclosure{min-width:var(--sv-touch-target)!important}.hub-settings-actions{display:grid;grid-template-columns:1fr 1fr}.hub-settings-actions .hub-settings-status{grid-column:1/-1;width:100%}.hub-settings-advanced>summary{padding:10px}.sv-feedback{min-height:var(--sv-touch-target)}}
@media(prefers-reduced-motion:reduce){.sv-feedback.is-loading::before{animation:none}}
</style>
...
<button id="openCalibrationProfilesButton" class="nav-card" type="button"><b>Calibration Profiles</b><span class="nav-points"><span>Manage Faceplate Calibrations</span><span>Import / Export Profiles</span><span>Clean Stale Profiles</span></span></button>
...
<p>Discovery validates the Native dashboard source and prepares manual exports without changing the Native dashboard. <b>New dashboard:</b> use <b>Copy Dashboard YAML</b> or <b>Download Dashboard YAML</b>, then paste it into Home Assistant's Raw configuration editor. <b>Existing dashboard:</b> use <b>Copy Cards Only</b> and paste the list entries beneath the target view's <code>cards:</code> key.</p>
<p class="muted">Manual exports are snapshots. A later Discovery run updates the Native generated source, not YAML already pasted into another dashboard. The full manual dashboard export removes the generated <code>custom:vertical-layout</code> wrapper so Layout Card is not required.</p>
...
function setFeedback(id,text,state='info'){const node=$(id);if(!node)return;node.textContent=String(text??'');node.classList.add('sv-feedback');for(const cls of ['is-info','is-loading','is-success','is-warning','is-error'])node.classList.remove(cls);const normalized=['info','loading','success','warning','error'].includes(state)?state:'info';node.classList.add(`is-${normalized}`);node.dataset.feedbackState=normalized;node.setAttribute('aria-live','polite')}
...
async function loadUnifi2mqttSettings(){setView('unifi2mqtt');setFeedback('unifiSettingsStatus','Loading UniFi2MQTT settings…','loading');try{const d=await fetchUnifi2mqttSettings();renderUnifi2mqttSettings(d);setFeedback('unifiSettingsStatus',d.failover_active?`Priority path unavailable — currently using ${d.active_transport||'the configured'} fallback. The priority path will be retried automatically.`:'Saved credentials are masked by default. Use the eye to reveal one on demand; blank fields preserve stored values.',d.failover_active?'warning':'info')}catch(e){setFeedback('unifiSettingsStatus',`Could not load UniFi2MQTT settings: ${e}`,'error')}}
...
async function saveUnifi2mqttSettings(){const btn=$('saveUnifi2mqttButton');btn.disabled=true;setFeedback('unifiSettingsStatus','Saving UniFi2MQTT settings…','loading');try{const r=await fetch(endpoint('api/unifi2mqtt/settings'),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(unifiSettingsPayload())});const d=await r.json();if(!r.ok)throw new Error(d.error||'Could not save UniFi2MQTT settings');renderUnifi2mqttSettings(d);setFeedback('unifiSettingsStatus',d.restarted?'Saved. UniFi2MQTT restart requested.':(d.started?'Saved. UniFi2MQTT start requested.':'Saved.'),'success');appLinksCache=null}catch(e){setFeedback('unifiSettingsStatus',`Could not save: ${e}`,'error')}finally{btn.disabled=false}}
async function installUnifi2mqtt(){const btn=$('installUnifi2mqttButton');btn.disabled=true;setFeedback('unifiInstallStatus','Installing UniFi2MQTT…','loading');try{const r=await fetch(endpoint('api/unifi2mqtt/install'),{method:'POST'});const d=await r.json();if(!r.ok)throw new Error(d.error||'Could not install UniFi2MQTT');appLinksCache=null;renderUnifi2mqttSettings(d);setFeedback('unifiInstallStatus','Installed. Configure the controller and MQTT settings below.','success')}catch(e){setFeedback('unifiInstallStatus',`Could not install: ${e}`,'error')}finally{btn.disabled=false}}
...
function detectedDeviceKey(item){return [item?.data_source||'SNMP',item?.name||'',item?.model||'Unknown model'].join('|')}function deviceSummaryFact(label,value){const fact=document.createElement('span');fact.className='device-summary-fact';const name=document.createElement('span');name.className='device-summary-fact-label';name.textContent=label;const shown=document.createElement('span');shown.className='device-summary-fact-value';shown.textContent=String(value??'—');fact.append(name,shown);return fact}
...
function renderUnifiedDevices(){const root=$('configuredDevices');if(!root)return;for(const entry of root.querySelectorAll('[data-device-key]')){const key=entry.dataset.deviceKey||'';if(!key)continue;const open=entry.tagName==='DETAILS'?entry.open:entry.dataset.expanded==='true';if(open)expandedUnifiedDevices.add(key);else expandedUnifiedDevices.delete(key)}root.innerHTML='';const config=lastConfiguredDevices||{};const devices=config.devices||[];const detected=lastDevicesDiagnostics?.devices||[];const used=new Set();const writable=!!config.writable;for(const [position,item] of devices.entries()){const enabled=item.enabled!=='disabled';const found=detectedForConfigured(item,detected,used);const detectedItem=found?.item||null;if(found)used.add(found.index);const key=`saved|${item.switch_name}`;const expanded=expandedUnifiedDevices.has(key);const entry=document.createElement('div');entry.className=`device-card unified-device-details configured-device${enabled?'':' disabled'}`;entry.dataset.deviceKey=key;entry.dataset.expanded=String(expanded);const summary=document.createElement('div');summary.className='unified-device-summary';const main=document.createElement('div');main.className='unified-device-main';const titleRow=document.createElement('div');titleRow.className='device-summary-title-row';const title=document.createElement('strong');title.textContent=configuredDeviceTitle(item);const health=document.createElement('span');health.className=`device-health-chip ${detectedItem?'is-enabled':'is-disabled'}`;health.textContent=detectedItem?'Detected':'Not detected';titleRow.append(title,health);const line=document.createElement('div');line.className='muted';const bits=[item.switch_name,item.sensor_prefix,item.switch_model&&item.switch_model!=='auto'?item.switch_model:'Auto-detect'].filter(Boolean);line.textContent=bits.join(' · ');const management=document.createElement('div');management.className='muted configured-management';const configured=item.configured_management_target||'Not configured';const effective=item.effective_management_target||(item.effective_management_status==='invalid_saved_row'?'Unavailable — saved row needs review':'Not configured');management.textContent=`Configured management IP/host: ${configured} · Effective management IP/host: ${effective}`;main.append(titleRow,line,management);const facts=document.createElement('div');facts.className='device-summary-facts';facts.append(deviceSummaryFact('Model',detectedItem?.model||(item.switch_model&&item.switch_model!=='auto'?item.switch_model:'Auto-detect')),deviceSummaryFact('Support',detectedItem?statusLabel(detectedItem.registry_status||'detected'):'Not detected'),deviceSummaryFact('Last refresh',lastDevicesDiagnostics?.generated_at||'Not refreshed'));main.append(facts);if(detectedItem){const detection=document.createElement('div');detection.className='muted detected-device-generated';detection.textContent=`Ports: ${detectedItem.rj45_interfaces||0} RJ45 + ${detectedItem.uplink_interfaces||0} uplinks`;main.append(detection)}else{const detection=document.createElement('div');detection.className='muted detected-device-generated';detection.textContent='Hardware detail is unavailable until a matching device is detected.';main.append(detection)}const actions=document.createElement('div');actions.className='unified-device-actions';if(detectedItem){const source=document.createElement('span');source.className='device-source-chip';source.textContent=detectedItem.data_source||'SNMP';actions.append(source)}const up=document.createElement('button');up.type='button';up.className='device-order-button';up.textContent='↑';up.dataset.writable=String(writable);up.dataset.boundary=String(position===0);up.setAttribute('aria-label',`Move up ${configuredDeviceTitle(item)}`);up.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();if(position>0)moveConfiguredDevice(item,devices[position-1],'up',up)});const down=document.createElement('button');down.type='button';down.className='device-order-button';down.textContent='↓';down.dataset.writable=String(writable);down.dataset.boundary=String(position===devices.length-1);down.setAttribute('aria-label',`Move down ${configuredDeviceTitle(item)}`);down.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();if(position<devices.length-1)moveConfiguredDevice(item,devices[position+1],'down',down)});const toggle=document.createElement('button');toggle.type='button';toggle.className=`device-state-toggle ${enabled?'enabled':'disabled'}`;toggle.dataset.writable=String(writable);toggle.setAttribute('role','switch');toggle.setAttribute('aria-checked',String(enabled));toggle.setAttribute('aria-label',`${enabled?'Disable':'Enable'} ${configuredDeviceTitle(item)}`);const track=document.createElement('span');track.className='toggle-track';track.setAttribute('aria-hidden','true');const knob=document.createElement('span');knob.className='toggle-knob';track.appendChild(knob);const label=document.createElement('span');label.textContent=enabled?'Enabled':'Disabled';toggle.append(track,label);toggle.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();setConfiguredDeviceState(item,enabled?'disabled':'enabled',toggle)});const chevron=document.createElement('span');chevron.className='unified-device-chevron';chevron.setAttribute('aria-hidden','true');chevron.textContent='▸';const disclosure=document.createElement('button');disclosure.type='button';disclosure.className='unified-device-disclosure';disclosure.setAttribute('aria-expanded',String(expanded));disclosure.setAttribute('aria-label',`${expanded?'Hide':'Show'} details for ${configuredDeviceTitle(item)}`);const detailLabel=document.createElement('span');detailLabel.className='device-details-label';detailLabel.textContent='Details';disclosure.append(detailLabel,chevron);const orderControls=document.createElement('div');orderControls.className='device-order-controls';orderControls.append(up,down);actions.append(toggle,disclosure);summary.append(orderControls,main,actions);const body=document.createElement('div');body.className='unified-device-body';if(detectedItem)appendDetectedDeviceBody(body,detectedItem,lastDevicesDiagnostics||{});else{const empty=document.createElement('p');empty.className='muted';empty.textContent='No current detected-device details are available for this saved switch. Run Discovery to refresh its hardware information.';body.append(empty)}body.id=`configured-device-details-${position}`;body.hidden=!expanded;disclosure.setAttribute('aria-controls',body.id);const setExpanded=open=>{entry.dataset.expanded=String(open);body.hidden=!open;disclosure.setAttribute('aria-expanded',String(open));disclosure.setAttribute('aria-label',`${open?'Hide':'Show'} details for ${configuredDeviceTitle(item)}`);if(open)expandedUnifiedDevices.add(key);else expandedUnifiedDevices.delete(key)};disclosure.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();setExpanded(entry.dataset.expanded!=='true')});entry.append(summary,body);root.append(entry)}for(const [index,item] of detected.entries()){if(used.has(index))continue;const key=`detected|${detectedDeviceKey(item)}`;const entry=document.createElement('details');entry.className='device-card unified-device-details';entry.dataset.deviceKey=key;entry.open=expandedUnifiedDevices.has(key);entry.addEventListener('toggle',()=>{if(entry.open)expandedUnifiedDevices.add(key);else expandedUnifiedDevices.delete(key)});const summary=document.createElement('summary');const main=document.createElement('div');main.className='unified-device-main';const titleRow=document.createElement('div');titleRow.className='device-summary-title-row';const title=document.createElement('strong');title.textContent=item.name||item.model||'Detected device';const health=document.createElement('span');health.className='device-health-chip is-enabled';health.textContent='Detected';titleRow.append(title,health);const line=document.createElement('div');line.className='muted';line.textContent=`${item.rj45_interfaces||0} RJ45 + ${item.uplink_interfaces||0} uplinks`;const facts=document.createElement('div');facts.className='device-summary-facts';facts.append(deviceSummaryFact('Model',item.model||'Unknown model'),deviceSummaryFact('Support',statusLabel(item.registry_status||'detected')),deviceSummaryFact('Last refresh',lastDevicesDiagnostics?.generated_at||'Not refreshed'));main.append(titleRow,line,facts);const side=document.createElement('div');side.className='unified-device-actions';const source=document.createElement('span');source.className='device-source-chip';source.textContent=item.data_source||'SNMP';const detailLabel=document.createElement('span');detailLabel.className='device-details-label';detailLabel.textContent='Details';const chevron=document.createElement('span');chevron.className='unified-device-chevron';chevron.setAttribute('aria-hidden','true');chevron.textContent='▸';side.append(source,detailLabel,chevron);summary.append(main,side);const body=document.createElement('div');body.className='unified-device-body';appendDetectedDeviceBody(body,item,lastDevicesDiagnostics||{});entry.append(summary,body);root.append(entry)}if(!devices.length&&!detected.length)root.innerHTML='<p class="muted">No saved or detected switches are available yet. Add a switch in Discovery Settings or run Discovery.</p>';syncConfiguredDeviceToggleAvailability()}
function renderConfiguredDevices(d){lastConfiguredDevices=d;renderUnifiedDevices();if(!d?.switch_list_enabled&&(d?.devices||[]).length)setFeedback('configuredDevicesStatus','The saved switch list is globally disabled in Discovery Settings.','warning');else if(!d?.writable)setFeedback('configuredDevicesStatus','Read-only fallback: Home Assistant app configuration is unavailable. Use Discovery Settings to change device state.','warning');else setFeedback('configuredDevicesStatus',`${d?.count||0} saved device(s). Device order is persistent and is reused by Discovery and YAML/Card regeneration.`,'info')}
async function moveConfiguredDevice(item,destination,direction,button){if(lastDiscoveryState?.running){setFeedback('configuredDevicesStatus','Stop Discovery before changing device order.','warning');return}button.disabled=true;const title=configuredDeviceTitle(item);setFeedback('configuredDevicesStatus',`Moving ${title} ${direction}…`,'loading');try{const r=await fetch(endpoint('api/configured-devices/order'),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({index:item.index,switch_name:item.switch_name,destination_index:destination.index,destination_switch_name:destination.switch_name})});const d=await r.json();if(!r.ok)throw new Error(d.error||'Could not save device order');renderConfiguredDevices(d);setFeedback('configuredDevicesStatus','Saved persistent device order. Discovery and YAML/Card regeneration will use this order.','success')}catch(e){setFeedback('configuredDevicesStatus',`Could not change device order: ${e.message||e}`,'error');await refreshConfiguredDevices(false)}finally{syncConfiguredDeviceToggleAvailability()}}
async function refreshConfiguredDevices(showStatus=false){if(showStatus)setFeedback('configuredDevicesStatus','Refreshing saved devices…','loading');try{const r=await fetch(endpoint('api/configured-devices'),{cache:'no-store'});const d=await r.json();if(!r.ok)throw new Error(d.error||'Could not load saved devices');renderConfiguredDevices(d)}catch(e){setFeedback('configuredDevicesStatus',`Could not load saved devices: ${e.message||e}`,'error')}}
async function setConfiguredDeviceState(item,nextState,button){if(lastDiscoveryState?.running){setFeedback('configuredDevicesStatus','Stop Discovery before changing device state.','warning');return}button.disabled=true;const title=configuredDeviceTitle(item);setFeedback('configuredDevicesStatus',`Saving ${title} as ${nextState}…`,'loading');try{const r=await fetch(endpoint('api/configured-devices/state'),{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({index:item.index,switch_name:item.switch_name,enabled:nextState})});const d=await r.json();if(!r.ok)throw new Error(d.error||'Could not save device state');renderConfiguredDevices(d);setFeedback('configuredDevicesStatus',`${title} is now ${nextState}. Regenerate Dashboard Card YAML to apply this saved state immediately; no Discovery run is required.`,'success')}catch(e){setFeedback('configuredDevicesStatus',`Could not change ${title}: ${e.message||e}`,'error');await refreshConfiguredDevices(false)}finally{syncConfiguredDeviceToggleAvailability()}}
...
async function refreshDevicesData(showStatus=true){if(showStatus)setFeedback('devicesActionStatus','Refreshing devices…','loading');try{const d=await fetchDiagnosticsData();window.latestDiagnostics=d;renderDevices(d);$('downloadDiagnosticsButton').href=endpoint('download/diagnostics.txt');if(showStatus)setFeedback('devicesActionStatus',`Updated ${d.generated_at||''}`,'success')}catch(e){if(showStatus)setFeedback('devicesActionStatus',`Could not load devices: ${e}`,'error')}}
...
async function copyDiagnostics(){setFeedback('devicesActionStatus','Copying diagnostics…','loading');try{const r=await fetch(endpoint('download/diagnostics.txt'),{cache:'no-store'});if(!r.ok)throw new Error(`Diagnostics download failed (${r.status})`);const text=await r.text();const copied=await copyTextWithFallback(text);if(!copied)throw new Error('Clipboard access is unavailable in this browser context');setFeedback('devicesActionStatus','Diagnostics copied to clipboard.','success')}catch(e){setFeedback('devicesActionStatus',`Could not copy diagnostics: ${e}`,'error')}}
...
async function loadGeneratedCardYamlStatus(){const status=$('generatedCardYamlActionStatus');const preview=$('generatedCardYamlPreview');try{const r=await fetch(endpoint('api/generated-card-yaml/status'),{cache:'no-store'});const d=await r.json();if(!r.ok)throw new Error(d.error||'Could not read generated Dashboard YAML status');const found=!!d.generated?.found;const valid=!!d.validation?.valid;const modified=d.generated?.modified||null;$('generatedCardYamlState').textContent=found?`${d.generated.path||'generated-dashboard-card.yaml'} · ${d.generated.size||0} bytes`:'Not generated';$('generatedCardYamlValidation').textContent=valid?`Valid · ${d.validation?.documents||1} YAML document${(d.validation?.documents||1)===1?'':'s'}`:`Invalid · ${d.validation?.error||'validation failed'}`;$('generatedCardYamlUpdated').textContent=modified||'Not available';$('downloadGeneratedDashboardYamlButton').href=endpoint('download/switch-vision-dashboard.yaml');if(!found||!valid){preview.textContent='';preview.classList.add('hidden');status.textContent=!found?'Run Discovery to generate Dashboard YAML.':`Generated Dashboard YAML is not available for export: ${d.validation?.error||'validation failed'}`}else{if(generatedCardYamlModified&&modified!==generatedCardYamlModified&&!preview.classList.contains('hidden')){preview.textContent=await fetchGeneratedDashboardExport('dashboard');status.textContent='Dashboard YAML export preview refreshed.'}else if(status.textContent.startsWith('Could not load')||status.textContent.startsWith('Generated Dashboard YAML is not available'))status.textContent=''}generatedCardYamlModified=modified}catch(e){preview.textContent='';preview.classList.add('hidden');status.textContent=`Could not load generated Dashboard YAML status: ${e}`}}
async function fetchGeneratedDashboardExport(mode='dashboard'){const path=mode==='cards'?'api/generated-card-yaml/cards-export':'api/generated-card-yaml/dashboard-export';const r=await fetch(endpoint(path),{cache:'no-store'});const d=await r.json();if(!r.ok)throw new Error(d.error||'Dashboard YAML export failed');return d.text||''}
async function previewGeneratedDashboardYaml(){const status=$('generatedCardYamlActionStatus');try{const text=await fetchGeneratedDashboardExport('dashboard');$('generatedCardYamlPreview').textContent=text;$('generatedCardYamlPreview').classList.remove('hidden');status.textContent='Dashboard YAML export preview loaded.'}catch(e){status.textContent=`Could not preview Dashboard YAML: ${e}`}}
async function copyGeneratedDashboardYaml(){const status=$('generatedCardYamlActionStatus');status.textContent='Copying Dashboard YAML…';try{const text=await fetchGeneratedDashboardExport('dashboard');const copied=await copyTextWithFallback(text);if(!copied)throw new Error('Clipboard access is unavailable in this browser context');$('generatedCardYamlPreview').textContent=text;$('generatedCardYamlPreview').classList.remove('hidden');status.textContent='Dashboard YAML copied. Paste it into a new Home Assistant dashboard Raw configuration editor.'}catch(e){status.textContent=`Could not copy Dashboard YAML: ${e}`}}
...
async function runDiscovery(){const btn=$('runDiscoveryButton');btn.disabled=true;setFeedback('discoveryStatus','Preparing Discovery','loading');try{const r=await fetch(endpoint('api/discovery/start'),{method:'POST'});const d=await r.json();if(!r.ok)throw new Error(d.error||'Could not start Discovery');await refresh()}catch(e){setFeedback('discoveryStatus',`Could not start Discovery: ${e}`,'error');btn.disabled=false}}
async function stopDiscovery(){const btn=$('stopDiscoveryButton');btn.disabled=true;setFeedback('discoveryStatus','Stopping Discovery','loading');try{const r=await fetch(endpoint('api/discovery/stop'),{method:'POST'});const d=await r.json();if(!r.ok)throw new Error(d.error||'Could not stop Discovery');await refresh()}catch(e){setFeedback('discoveryStatus',`Could not stop Discovery: ${e}`,'error');btn.disabled=false}}
