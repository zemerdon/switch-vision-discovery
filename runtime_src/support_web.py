...
<div class="actions"><button id="previewGeneratedDashboardYamlButton" type="button">Preview Dashboard YAML</button><label>New dashboard format<select id="generatedDashboardExportMode"><option value="custom-dashboard" selected>Custom dashboard (preserve vertical layout)</option><option value="dashboard">Standard dashboard (no Layout Card)</option></select><small>Preview, Copy, and Download use this format. Cards Only is unchanged.</small></label><button id="copyGeneratedDashboardYamlButton" type="button">Copy Dashboard YAML</button><button id="copyGeneratedCardsOnlyButton" type="button">Copy Cards Only</button><a id="downloadGeneratedDashboardYamlButton" class="button" href="#">Download Dashboard YAML</a></div>
...
<button id="devicesTab-autodiscover" class="hub-settings-tab" type="button" role="tab" aria-selected="false" aria-controls="devicesPanel-autodiscover" data-devices-tab="autodiscover">AutoDiscover</button>
...
<p class="lead">Prepare or re-download privacy-processed contribution bundles. Nothing is sent automatically.</p>
<div class="hub-settings-tabs" role="tablist" aria-label="Support My Switch sections">
<button id="supportTab-create" class="hub-settings-tab active" type="button" role="tab" aria-selected="true" aria-controls="supportPanel-create">Create</button>
<button id="supportTab-history" class="hub-settings-tab" type="button" role="tab" aria-selected="false" aria-controls="supportPanel-history">History</button>
</div>
<div id="supportPanel-create" role="tabpanel" aria-labelledby="supportTab-create">
...
<div id="supportPanel-history" role="tabpanel" aria-labelledby="supportTab-history" hidden>
<h3>Contribution history</h3>
<p class="muted">Previously generated privacy-processed bundles retained on this Home Assistant host. Re-download an existing bundle instead of rebuilding it when the evidence you need has not changed.</p>
<div class="actions"><button id="refreshContributionHistoryButton" type="button">Refresh History</button></div>
<p id="contributionHistoryStatus" class="muted">History has not been loaded yet.</p>
...
function contributionHistoryDate(value){if(!value)return'Unknown';const d=new Date(value);return Number.isNaN(d.getTime())?String(value):new Intl.DateTimeFormat(undefined,{dateStyle:'medium',timeStyle:'short'}).format(d)}
function contributionDownloadButton(label,name,primary=false){const a=document.createElement('a');a.className=`button${primary?' primary':''}`;a.textContent=label;a.href=endpoint(`download/${encodeURIComponent(name)}`);a.setAttribute('download','');return a}
function renderContributionHistory(items){const root=$('contributionHistoryList'),status=$('contributionHistoryStatus');root.innerHTML='';if(!Array.isArray(items)||!items.length){status.textContent='No retained contribution bundles were found.';return}status.textContent=`${items.length} retained contribution${items.length===1?'':'s'}`;for(const item of items){const card=document.createElement('div');card.className='device-card';const head=document.createElement('div');head.className='device-head';const title=document.createElement('div');const strong=document.createElement('strong');strong.textContent=item.contribution_id||'Unknown contribution';title.append(strong);const sub=document.createElement('div');sub.className='muted';sub.textContent=`Switch Vision v${item.version||'Unknown'} · ${contributionHistoryDate(item.created_at||item.modified_at)}`;title.append(sub);const badge=document.createElement('span');badge.className=`badge ${item.ready_to_send?'badge-supported':'badge-detected'}`;badge.textContent=item.ready_to_send?'Ready':'Review';head.append(title,badge);card.append(head);const meta=document.createElement('div');meta.className='device-meta-line';for(const [label,value] of [['Quality',item.quality||'Unknown'],['Archive',fmtBytes(Number(item.archive_size)||0)],['Devices',String(item.device_count||0)]]){const span=document.createElement('span');const b=document.createElement('b');b.textContent=`${label}:`;span.append(b,document.createTextNode(` ${value}`));meta.append(span)}card.append(meta);if(Array.isArray(item.models)&&item.models.length){const models=document.createElement('p');models.className='muted';models.textContent=`Models: ${item.models.join(', ')}`;card.append(models)}const actions=document.createElement('div');actions.className='actions';if(item.archive)actions.append(contributionDownloadButton('Download ZIP',item.archive,true));if(item.email)actions.append(contributionDownloadButton('Download EML',item.email));if(item.actions)actions.append(contributionDownloadButton('Download Actions HTML',item.actions));card.append(actions);root.append(card)}}
async function loadContributionHistory(){const status=$('contributionHistoryStatus');status.textContent='Loading contribution history…';try{const r=await fetch(endpoint('api/contributions/history'),{cache:'no-store'});const d=await r.json();if(!r.ok)throw new Error(d.error||'Could not load contribution history');renderContributionHistory(d.items||[])}catch(e){status.textContent=`Could not load contribution history: ${e.message||e}`;}}
function setSupportTab(tab){const history=tab==='history';$('supportTab-create').classList.toggle('active',!history);$('supportTab-history').classList.toggle('active',history);$('supportTab-create').setAttribute('aria-selected',String(!history));$('supportTab-history').setAttribute('aria-selected',String(history));$('supportPanel-create').hidden=history;$('supportPanel-history').hidden=!history;const primary=$('hubSharedPrimary');if(currentView==='support'&&primary){primary.textContent=history?'Refresh History':'Create Contribution';primary.dataset.target=history?'refreshContributionHistoryButton':'createButton'}if(history)loadContributionHistory()}
...
let lastAutoDiscoverResults=null;
...
'Slow blink period (ms)':'Controls the visual cadence for Slow activity. Allowed range: 120–2000 ms. Lower values make light-traffic flicker more frequent; higher values make it calmer. The Test LED preview uses only the last successfully saved value.',
'Medium blink period (ms)':'Controls the visual cadence for Medium activity. Allowed range: 120–2000 ms. Lower values make the activity flicker more frequently. The Test LED preview uses only the last successfully saved value.',
'Fast blink period (ms)':'Controls the visual cadence for Fast activity. Allowed range: 120–2000 ms. Lower values produce the most rapid activity flicker. The Test LED preview uses only the last successfully saved value.',
...
def _autodiscover_registry() -> dict[str, Any]:
    data = _read_json(DEFAULT_REGISTRY_FILE)
    if isinstance(data, dict):
        return data
    if isinstance(data, list):
        return {"devices": data}
...
def _autodiscover_unifi_snapshot() -> dict[str, Any]:
    data = _read_json(DEFAULT_UNIFI_SNAPSHOT)
...
def _autodiscover_status() -> dict[str, Any]:
    ...
    return {
        ...
        "saved_credentials": [
            {"ref": str(item.get("ref") or ""), "label": str(item.get("label") or "")}
...
def _autodiscover_scan(data: Any) -> dict[str, Any]:
    ...
    result = autodiscover.scan(
        ...
        use_saved=use_saved,
        registry_data=_autodiscover_registry(),
        unifi_snapshot=_autodiscover_unifi_snapshot(),
        timeout=timeout,
    )
...
def _autodiscover_add(data: Any) -> dict[str, Any]:
    ...
    if not isinstance(requested, list) or not requested or len(requested) > 256:
        raise ValueError("AutoDiscover add requires between 1 and 256 candidate devices.")

    options = _effective_discovery_options(_self_addon_options())
    current_rows = options.get("switches") if isinstance(options.get("switches"), list) else []
    configured_hosts = {
    ...
    registry = _autodiscover_registry()
    try:
        configured_timeout = float(str(options.get("snmp_timeout") or "1"))
    except ValueError:
        configured_timeout = 1.0
    ...
    for index, raw in enumerate(requested, start=1):
        ...
        if unexpected:
            raise ValueError(f"Unsupported AutoDiscover candidate field: {unexpected[0]}")
        host = _plain_text(raw.get("host", ""), f"AutoDiscover candidate {index} host", max_length=64, allow_empty=False).strip()
        try:
        ...
        row = {
            ...
            "enabled": "enabled",
            "walk_mode": "targeted",
            # AutoDiscover may show a preliminary model hint, but normal
            # Discovery owns exact identification after the device is saved.
            "switch_model": "auto",
            "card_header_title": "",
        }
        additions.append(_validate_switch_row(row, len(current_rows) + len(additions) + 1))
        added_safe.append(
            {
                "host": host,
                "switch_name": switch_name,
                "display_name": display_name,
                "model_hint": str(verified.get("model") or verified.get("model_hint") or "")[:160],
            }
        )
        seen_hosts.add(host)

    # Remove only the factory placeholder when at least one real switch is
    # being added. Existing real saved rows, including disabled rows, remain.
    retained = [
        dict(row)
        for row in current_rows
        if isinstance(row, dict)
    ...
    _save_discovery_settings({"settings": {"switches": retained + additions}})
    return {
        "ok": True,
